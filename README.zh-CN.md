...
一键部署会为 EdgeEver 产品 Worker 创建专属 GitHub 仓库和所需 Cloudflare 资源，并在仓库每日同步上游版本后自动触发重新部署；配置与排错见[使用 Cloudflare 一键部署 EdgeEver](docs/deploy-cloudflare-button.zh-CN.md)。`apps/site` 中的官方产品官网是独立的 Cloudflare Pages 项目，自托管产品实例不需要部署它。
...
Agent 应优先按 [AI Agent Cloudflare Deployment](docs/agent-deploy-cloudflare.md) 执行。首次部署后，请参阅 [Cloudflare Workers Builds 自动部署](docs/cloudflare-workers-builds.zh-CN.md)。所有安装入口共用产品 Worker 的构建、migration、发布和验证流水线，不会部署官方产品官网。
